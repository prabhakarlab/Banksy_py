...
def load_multisamples_as_one(sample):
    ...
    spatial = pd.merge(
        adata.obs, spatial, how="left", left_index=True, right_index=True
    )  # match order in adata object
    ...
    manual = pd.merge(
        adata.obs, manual, how="left", left_index=True, right_index=True
    )  # match order in adata object
...
scatterplot = plt.scatter(
    adata.obs["x_pixel"], adata.obs["y_pixel"], c=[int(x) for x in adata.obs["sample"]]
)
...
coords_df["x_pixel"] = coords_df.groupby("sample")["x_pixel"].transform(
    lambda x: x - x.min()
)
...
r_hvg = pd.read_csv(
    os.path.join("data", "DLPFC", "harmony", "Rvig_harmony2k.csv")
...
adata.obsm[xy_coord] = np.vstack(
    (adata.obs[x_coord].values, adata.obs[y_coord].values)
).T
...
banksy_dict, banksy_matrix = generate_banksy_matrix(
    adata, banksy_dict, lambda_list, max_m=m
)
...
for pca_dim in pca_dims:
    ...
    banksy_dict[nbr_weight_decay][0.2]["adata"].obsm[f"reduced_pc_{pca_dim}_umap"] = (
        umap_embedding
    )
...
grid = fig.add_gridspec(
    ncols=3, nrows=2
)  # modify columns and row of final plot base on expected output figure
    ...
    for params_name in results_df.index:
        if params_name == "scaled_gaussian_pc20_nc0.20_r0.40":
            ...
            scatterplot = ax_locs1.scatter(
                adata_plt_temp.obs[coord_keys[0]],
...
for counter, sample in enumerate(samples):
    for params_name in results_df.index:
        ...
        adata_ari_temp.obs["manual_annotations"] = adata_ari_temp.obs[
            "manual_annotations"
        ].astype("category")  # convert to categorical data for ARI computation
...
for counter, sample in enumerate(samples):
    for counter2, params_name in enumerate(target_params):
        ...
        adata_ari_temp.obsm["coord_xy"] = adata.obsm["coord_xy"][
            adata.obs["sample"] == sample
        ]
