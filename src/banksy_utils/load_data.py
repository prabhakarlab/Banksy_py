...
def load_adata(
    ...
    if load_adata_directly:
        ...
        if os.path.isfile(os.path.join(filepath, adata_filename)):
            ...
            print(
                f"No such files {adata_filename} in {filepath}, please check the directory path and file names"
            )
            print(
                f"Alternatively, try to convert raw files to anndata if by setting 'load_adata_directly = False'"
            )
    else:
        ...
        try:
            ...
            locations_df = pd.read_csv(
                os.path.join(filepath, locations_filename), index_col=0
            )
            print(
                f"GCM data successfully read as {gcm_df}\n Location data successfuly read as {locations_df}"
            )
        except:
            Exception(
                "Error occured when reading csv files, check the if files are permissible to read"
            )

        sparse_X = sparse.csc_matrix(gcm_df.values.T)

        adata = AnnData(
            X=sparse_X, obs=locations_df, var=pd.DataFrame(index=gcm_df.index)
        )
    ...
    try:
        print(
            f"Attempting to concatenate spatial x-y under adata.obsm[{coord_keys[2]}]"
        )
        ...
        print(
            f"Error in concatenating the matrices under adata.obsm[{coord_keys[2]}]\n raw_x, raw_y will return None"
        )
...
def display_adata(adata: anndata.AnnData) -> None:
    ...
    display(adata)
    print(
        f"Matrix sparsity: {adata.X.nnz} filled elements "
