...
def generate_spatial_distance_graph(
    ...
    if num_neighbours is None:
        ...
        graph_out = nbrs.kneighbors_graph(n_neighbors=num_neighbours, mode="distance")
        ...
        if radius is not None:
            ...
            graph_out = remove_greater_than(
                graph_out, radius, copy=False, verbose=False
            )
...
def generate_spatial_weights_fixed_radius(
    ...
    if decay_type == "gaussian":
        ...
        if verbose:
            print(
                f"Equivalent radius for removing {p} of "
    ...
    distance_graph = generate_spatial_distance_graph(
        locations, nbr_object=nbr_object, num_neighbours=max_num_neighbours, radius=r
    )
...
def concatenate_all(
    matrix_list: List[Union[np.ndarray, csr_matrix]],
    ...
    for k in range(num_k):
        scale_factors_squared[k + 1] = (
            1 / (2 ** (k + 1)) / denom * neighbourhood_contribution
        )

    scale_factors = np.sqrt(scale_factors_squared)

    print(
        f"Scale factors squared: {scale_factors_squared}\nScale factors: {scale_factors}"
    )
...
def weighted_concatenate(
    cell_genes: Union[np.ndarray, csr_matrix],
...
class LeidenPartition(object):
    ...
    def __init__(
        ...
        if self.nns_have_weights:
            ...
            self._print_csr_info(self.nn_connectivity, "Nearest-neighbour connectivity")
        ...
        if compute_shared_nn:
            ...
            if self.nns_have_weights:
                ...
                self._print_csr_info(
                    self.snn_weighted, "shared NN with distance-based weights"
                )
                print(f"shared NN weighted graph data: {self.snn_weighted.data}")
                self.G = self.csr_to_igraph(
                    self.snn_weighted,
                )
    ...
    def shared_nn(
        ...
        if self.verbose:
            self._print_csr_info(
                snn_connections, "Shared nearest-neighbour (connections only)"
            )
            self._print_csr_info(
                snn_graph,
    ...
    def csr_to_igraph(
        ...
        return igraph.Graph(edgelist, edge_attrs={"weight": csr_mat.data.tolist()})
...
@timer
def median_dist_to_nearest_neighbour(adata: anndata.AnnData, key: str = "coord_xy"):
