...
from banksy_utils.cluster_utils import (
    pad_clusters,
    get_DEgenes,
    get_metagene_difference,
)
...
def plot_qc_hist(
    ...
    **kwargs,
    ...
    options = {"figsize": (15, 4), "bin_options": ["auto", "auto", "auto", "auto"]}
    ...
    sns.histplot(
        adata.obs["total_counts"], kde=False, bins=bin_sizes[0], ax=axs[0]
    ).set(title="Total Counts (unbounded)")

    sns.histplot(
        adata.obs["total_counts"][adata.obs["total_counts"] < total_counts_cutoff],
        kde=False,
        bins=bin_sizes[1],
        ax=axs[1],
    ).set(title=f"Total Counts (bounded)")

    sns.histplot(
        adata.obs["n_genes_by_counts"], kde=False, bins=bin_sizes[2], ax=axs[2]
    ).set(title="n-genes by counts (unbounded)")
    ...
    sns.histplot(
        ...
        bins=bin_sizes[3],
        ax=axs[3],
    ).set(title="n-genes by counts (bounded)")
...
def plot_cell_positions(
    ...
    **kwargs,
    ...
    ax.scatter(
        adata.obs[coord_keys[0]],
        adata.obs[coord_keys[1]],
        s=options["s"],
        c=options["c2"],
...
def plot_theta_graph(
    ...
    **kwargs,
    ...
    for cell in randint(1, adata.shape[0], 1):
        ...
        ax.plot(
            adata.obsm[key][cell, 0],
            adata.obsm[key][cell, 1],
            "bo",
            alpha=options["alpha"],
        )
        ptr_start = theta_graph.indptr[cell]
        ptr_end = theta_graph.indptr[cell + 1]
        for nbr, angle in zip(
            theta_graph.indices[ptr_start:ptr_end], theta_graph.data[ptr_start:ptr_end]
        ):
            ax.plot(
                adata.obsm[key][nbr, 0],
                adata.obsm[key][nbr, 1],
                "ro",
                alpha=options["alpha"],
            )
            ax.annotate(
                f"{angle / np.pi * 180:0.1f}",
    ...
    for adata in [adata_spatial, adata_nonspatial]:
        ...
        if plot_heatmap:
            sc.pl.rank_genes_groups_heatmap(
                adata, n_genes=6, vmin=-3, vmax=3, cmap="bwr", swap_axes=True
            )
        if plot_dot_plot:
...
def plot_DE_genes_refined(
    ...
    **kwargs,
    ...
    options = {"fig_size": (12, 5), "save_format": "eps", "dpi": 1200}
    options.update(kwargs)

    # filter AnnData object for only self-expresssion features
    adata_self = _adata_filter_self(
        adata_spatial,
    ...
    if plot_dotplot:
        ...
        if save_fig:
            fig2.savefig(
                os.path.join(
                    file_path, save_name + "dotplot." + options["save_format"]
                ),
                format=options["save_format"],
                dpi=options["dpi"],
            )
...
def plot_connection_grid(
    ...
    **kwargs,
    ...
    fig, axs = plt.subplots(
        nrows=1, ncols=2, figsize=(len(cell_types) * 2.4 + 3, len(cell_types) * 1.2)
    )
    ...
    for adata in [adata_nonspatial, adata_spatial]:
        ...
        for cell_type in cell_types:
            one_hots.append(np.array(adata.obs[cell_types_colname] == cell_type))
    ...
    for n, adata in enumerate([adata_spatial, adata_nonspatial]):
        ...
        for layer in adata.obs[clusters_colname].cat.categories:
            if layer in colors_dict:
                c = colors_dict[layer]
                locations = adata.obsm[coords_key][
                    adata.obs[clusters_colname] == layer, :
                ]
                # umap_embedding = adata.obsm[umap_name][adata.obs[clusters_colname] == layer, :]
                loc_plot = ax_loc.scatter(
                    locations[:, 0], locations[:, 1], c=c, s=0.2, alpha=1.0
                )
...
def _plot_embedding(
    embedding: str,  # key for obsm entry in an AnnData object
    ax: plt.Axes,
    adata: AnnData,
    cluster_subset: list,
    spot_size: float,
    ...
    for cluster in adata.obs[cluster_colname].cat.categories:
        if cluster not in cluster_subset:
            ...
            scatterplot = ax.scatter(
                locations[:, 0], locations[:, 1], c=c, s=spot_size, alpha=1.0
            )
...
def plot_markergene_sets(
    ...
    **kwargs,
    ...
    """
    options = {"subplot_size": (3, 3), "save_format": "png", "dpi": 1200}
    options.update(kwargs)
    ...
    for n, layer in enumerate(reference_dict):
        ...
        if layer in markergenes_dict:
            ...
            _plot_continuous(
                metagene_df[coord_keys[0]],
                metagene_df[coord_keys[1]],
                c,
                ax,
                vmax=1,
                title=layer + "\nmarker genes",
...
def plot_cluster_subset(
    ...
    if plot_umap:
        ...
        _plot_embedding(
            umap_name, axins, adata, cluster_subset, spot_size, colours_dict
        )
...
def plot_weights_comparsion(
    ...
    **kwargs,
    ...
    for col, cluster_key in enumerate(cluster_keys):
        ...
        if (
            rctd_coord is not None
            and rctd_weights is not None
        ...
        if markergenes_dict is not None and adata_allgenes is not None:
            ...
            for sc_clust in cluster_key[-1]:
                ...
                markers_temp = [
                    m
                    for m in markergenes_dict[sc_clust]
                    if m in adata_allgenes.var.index
                ]

                if markers_temp < markergenes_dict[sc_clust]:
                    print(
                        f"{len(markergenes_dict[sc_clust]) - len(markers_temp)} "
            ...
            _plot_continuous(
                adata_spatial.obs["xcoord"],
                adata_spatial.obs["ycoord"],
                metagene,
                ax,
                vmax=1,
                plot_cbar=False,
    ...
    if save_fig:
        if save_png_name:
            fig.savefig(
                os.path.join(file_path, save_png_name), format="png", dpi=options["dpi"]
            )
        if save_eps_name:
            fig.savefig(
                os.path.join(file_path, save_eps_name), format="eps", dpi=options["dpi"]
            )
...
def compare_weights(
    ...
    **kwargs,
    ...
    """
    options = {"file_path": "data/slide_seq/v1", "dpi": 1200}
    options.update(kwargs)
    ...
    for comparison_df, s in [(rctd_df, "RCTD"), (metagene_df_intersect, "Metagene")]:
        ...
        for adata, s2 in [
            (adata_nonspatial_rctd, "non-spatial"),
            (adata_spatial_rctd, "spatial"),
        ]:
            correlation_grid = np.zeros(
                (len(cell_types), len(reference)),
            )
            ...
            for col, ref in enumerate(reference):
                for row, compare in enumerate(cell_types):
                    mask = (
                        (adata.obs[cell_type_colname] == compare).astype(float).values
                    )
            ...
            if save_fig:
                fig.savefig(
                    os.path.join(options["file_path"], f"corr_to_{s}_{s2}.eps"),
                    format="eps",
                    dpi=options["dpi"],
                )


def plot_self_vs_nbr(
    adata_spatial: anndata.AnnData,
    ...
    for mask, colour, cluster in [
        (mask_1, "red", cell_type),
        (mask_2, "blue", cell_type2),
    ]:
...
def plot_self_vs_nbr_metagene_all(
    ...
    for m in range(max_m + 1):
        ...
        cell_types = [
            cluster
            for cluster in adata_self.obs[cell_type_colname].cat.categories
        ...
        for n, cell_type in enumerate(cell_types):
            ...
            for mask, colour, cluster in [
                (mask_1, "red", reference_cluster),
                (mask_2, "blue", cell_type),
            ]:
                self_expr = np.sqrt(1 - lambda_param) * metagene[mask]
                nbr_expr = np.sqrt(lambda_param) * metagene_nbr[mask]
                ...
                metagene_plot = ax.scatter(
                    self_expr, nbr_expr, c=colour, s=0.6, alpha=0.1, label=cluster
                )

                dfs.append(
                    pd.DataFrame(
                        data={"self": self_expr, "nbr": nbr_expr, "cluster": cluster}
                    )
                )
            ...
            sns.kdeplot(
                ...
                palette=["red", "blue"],
                alpha=0.5,
                common_norm=False,  # linewidth = 0.2, #common_grid=True,
                bw_adjust=1.2,
                legend=False,
                # multiple="stack",
                ax=ax_top,
