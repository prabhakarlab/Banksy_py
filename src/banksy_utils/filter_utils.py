...
def feature_selection(
    ...
    if svgs and hvgs:
        ...
        hvg_genes = get_hvgs(dcp(adata), top_hvgs=hvgs, flavor="seurat")
        ...
        adata = adata[
            :,
            adata.var["genename"].isin(hvg_genes)
            | adata.var["genename"].isin(svg_genes),
        ]
        print(
            f"Number of filtered dataset in SVGs {adata.var['genename'].isin(svg_genes).value_counts()}"
        )
        print(
            f"Number of filtered dataset in HVGs {adata.var['genename'].isin(hvg_genes).value_counts()}"
        )
    ...
    ### Filter HVGs by the top 1000
        ...
        if load_preprocesssed:
            ...
            hvg_genes = get_hvgs(dcp(adata), top_hvgs=hvgs, flavor="seurat_v3")
    ...
    if save_genes:
        ...
        np.savetxt(title, gene_ids, delimiter=", ", fmt="% s")
...
def filter_hvg(adata: anndata.AnnData, n_top_genes: int, flavor: str = "seurat"):
    ...
    if flavor.__eq__("seurat"):
        ...
        sc.pp.highly_variable_genes(adata_log1p, flavor=flavor, n_top_genes=n_top_genes)
        ...
        print(
            f"Flavour: {flavor} is not recognized, please use either 'seurat' or 'seurat_v3'"
        )
        return
