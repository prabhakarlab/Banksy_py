...
def plot_edge_histogram(
    graph: csr_matrix, ax: mpl.axes.Axes, title: str = "edge weights", bins: int = 100
):
    """
    ...
    print(f"\nEdge weights ({title}): median = {median_dist}, mode = {mode_dist}\n")
...
def plot_2d_embeddings(
    ...
    **kwargs,
    ...
    ax.set_title(f"{method_str} {space_str}", fontsize=title_fontsize)
    ...
    if plot_cmap:
        fig.colorbar(scatterplot, boundaries=np.arange(num_labels + 1) - 0.5).set_ticks(
    ...
    for gene_index, gene in enumerate(genes):
        ...
        if results_df is not None:
            ...
            for box_num in range(num_boxes):
                rect = Rectangle(
                    (x_upper, y_lower + y_step * box_num),
                    x_step,
                    y_step,
                    linewidth=1,
                    facecolor=cmap(pvals[box_num]),
                    edgecolor="gray",
                    clip_on=False,
                )
                ax.add_patch(rect)
...
def plot_labels_seperately(
    ...
    for n, label_id in enumerate(label.ids):
        ...
        ax_umap.set_title(
            f"{label_id} {embedding_str}",
            fontsize=4 * subplot_size[0],
            fontweight="bold",
        )
...
def plot_label_subset(
    labels_to_plot: Collection,
    label: Label,
    locations: np.ndarray,
    ...
    labels_to_plot = [label_id for label_id in labels_to_plot if label_id in label.ids]
