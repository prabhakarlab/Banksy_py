...
def run_mclust_partition(
    ...
    **kwargs,
    ...
    for nbr_weight_decay in banksy_dict:
        ...
        for lambda_param in banksy_dict[nbr_weight_decay]:
            ...
            for pca_dim in pca_dims:
                ...
                print(
                    f"Clustering PC_Embedding in obsm: {used_obsm}\n of type: {type(adata_temp.obsm[used_obsm])}"
                )
                ...
                if (options["no_annotation"]) and (not annotations):
                    ...
                    results[param_str] = {
                        ...
                        "ari": adjusted_rand_score(
                            annotations.dense, adata.obs["mclust"]
                        ),
                    }
    ...
    results_df, max_num_labels = convert2df(
        results, match_labels=False, resolution=False
    )
...
def mclust_R(
    ...
    print(
        f"obsm {used_obsm} : of type {type(embedding_np_array)} | shape {embedding_np_array.shape}"
    )
        ...
        for lambda_param in banksy_dict[nbr_weight_decay]:
            ...
            for pca_dim in pca_dims:
                ...
                if max_labels and not resolutions:
                    print(
                        "No resolutions indicated, trying to search for a suitable resolution from labels"
                    )
                    print(f"Finding resolution that matches {max_labels}")
                    resolutions = find_resolutions(
                        partitioner, max_labels, num_iterations, partition_seed
                    )
                ...
                for resolution in resolutions:
                    ...
                    if not annotations:
                        ...
                        results[param_str] = {
                            ...
                            "labels": label,
                            "adata": banksy_dict[nbr_weight_decay][lambda_param][
                                "adata"
                            ],
                        }
...
def convert2df(
    results: dict, match_labels: bool, resolution: bool = True
) -> pd.DataFrame:
    """
    ...
    if match_labels:
        results_df["relabeled"], max_num_labels = match_label_series(
            results_df["labels"], extra_labels_assignment="greedy", verbose=False
        )
