...
def concatenate_all(matrix_list: List[Union[np.ndarray, csr_matrix]],
    ...
    # contrib_sum = 0  # sum of lambdas from every k
    # contrib_temp = neighbourhood_contribution
    # scale_factors = np.zeros(len(matrix_list))
    # for k in range(num_k):
    #     scale_factors[k+1] = np.sqrt(contrib_temp)
    #     contrib_sum += contrib_temp
    #     contrib_temp /= 2
    # scale_factors[0] = np.sqrt(1 - contrib_sum)
    #
