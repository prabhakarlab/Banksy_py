...
def pad_clusters(
    clust2annotation: dict, original_clusters: list, pad_name: str = "other"
):
    """
...
def refine_cell_types(adata_spatial, adata_nonspatial, cluster2annotation_refine: dict):
    """
    ...
    for adata in [adata_nonspatial, adata_spatial]:
        ...
        adata.obs["cell type refined"] = (
            adata.obs["cell type"].map(cluster2annotation_refine).astype("category")
        )

    # remove other category
    adata_spatial_filtered = adata_spatial[
        adata_spatial.obs["cell type refined"] != "other", :
    ]
    adata_nonspatial_filtered = adata_nonspatial[
        adata_nonspatial.obs["cell type refined"] != "other", :
    ]

    return adata_spatial_filtered, adata_nonspatial_filtered


def create_metagene_df(
    ...
    for layer in markergenes_dict:
        markers = [
            marker
            for marker in markergenes_dict[layer]
        ...
        if markers < markergenes_dict[layer]:
            print(
                f"{len(markergenes_dict[layer]) - len(markers)} "
...
def get_DEgenes(adata, cell_type, top_n=20, verbose=True):
    DE_genes = sc.get.rank_genes_groups_df(adata, group=cell_type)["names"][
        :top_n
    ].tolist()
    if verbose:
...
def create_spatial_nonspatial_adata(
    ...
    adata_spatial.obs["cell type"] = (
        adata_spatial.obs[label_name].map(cluster2annotation_spatial).astype("category")
    )
    ...
    label_name = f"labels_{params_name}"
    adata_nonspatial.obs["cell type"] = (
        adata_nonspatial.obs[label_name]
        .map(cluster2annotation_nonspatial)
        .astype("category")
    )
