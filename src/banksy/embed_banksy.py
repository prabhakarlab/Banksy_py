...
def generate_banksy_matrix(
    ...
    for nbr_weight_decay in banksy_dict:
        ...
        for lambda_param in lambda_list:
            ...
            banksy_dict[nbr_weight_decay][lambda_param] = {
                "adata": banksy_matrix,
            }
            ...
            if save_matrix:
                try:
                    file_name = (
                        f"adata_{nbr_weight_decay}_l{lambda_param}_{time_str}.csv"
                    )
...
def create_nbr_matrix(
    ...
    for m in range(1, max_m + 1):
        ...
        def fscale_exp(X_dense, ind_temp):
            fscaled_exp = (
                X_dense[ind_temp,] - np.mean(X_dense[ind_temp,], axis=0)
            )  # raw expression - mean of expression of each cell in the n-cell neighborhood
            return fscaled_exp
        ...
        for n in range(weights.indptr.shape[0] - 1):
            ...
            if center:
                nbr_mat[n, :] = np.absolute(
                    np.dot(
                        weights_data[weights.indptr[n] : weights.indptr[n + 1]],
                        fscale_exp(X_dense, ind_temp),
                    )
                )  # cell expression now contain AGF weights
            else:
                nbr_mat[n, :] = np.absolute(
                    np.dot(
                        weights_data[weights.indptr[n] : weights.indptr[n + 1]],
    ...
    if variance_balance:
        for m in range(0, max_m + 1):
            ...
            pca_nbr.fit_transform(
                np.nan_to_num(sp.stats.zscore(nbr_mat, axis=0), nan=0)
            )
            ...
            if verbose:
                ...
                print(
                    f"Variance of top {n_components} PCs of nbrs' expression {round(sum_of_nbr_pca_var, 2)}"
                )
                print(
                    f"Multiplied nbrs' expression by balance factor: {round(balance_factor, 2)}"
                )
                print(f"After, Variance of Nbr_mat = {round(nbr_matrices[m].var(), 2)}")

    gc.collect()
    print(
        f"Ran 'Create BANKSY Matrix' in {round((time.perf_counter() - start_time) / 60, 2)} mins"
    )
    return nbr_matrices


def create_mat_list(adata, nbr_matrices, max_m: int):
...
def plot_std_per_gene(st_dev_pergene, lambda_param, **kwargs):
    ...
    options = {"figsize": (8, 2), "width": 1, "color": "slateblue"}
