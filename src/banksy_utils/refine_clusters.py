...
def refine_clusters(
    ...
    for params_name in results_df.index:
        ...
        if coord_keys[2] not in adata_temp.obsm_keys():
            adata_temp.obsm[coord_keys[2]] = pd.concat(
        ...
        if refine_method.__eq__("auto"):
            ...
            while (total_entropy * 100 > 5) and (num_iter < 20):
                refined_list, refined_ari, total_entropy = refine_once(
                    adata_temp, refined_list, annotations, coord_keys, num_neigh
                )
                if verbose:
                    print(
                        f"Refined iteration: {num_iter} | Total Entropy: {round(total_entropy, 2)}"
                    )
                num_iter += 1
            ...
            for i in range(refine_iterations):
                ...
                if verbose:
                    print(
                        f"Refined iteration: {i} | Total Entropy: {round(total_entropy, 2)}"
                    )
        ...
        subplot_sc(ax1, adata_temp, coord_keys, raw_clusters, num_clusters, title1)

        for i in refined_list:
            refined_clusters.append(color_list[i])

        subplot_sc(ax2, adata_temp, coord_keys, refined_clusters, num_clusters, title2)
    ...
    gc.collect()
    print(
        f"Time taken for refinement = {round((time.perf_counter() - start_time) / 60, 2)} min"
    )
    return results_df
...
def refine_once(
    ...
    refined_ari = (
        adjusted_rand_score(refined_list, truth_labels) if truth_labels else -1
    )
