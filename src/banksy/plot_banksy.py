...
def plot_results(
    ...
    **kwargs,
    ...
    options = {"save_all_h5ad": False, "group_num": 20}
    ...
    for params_name in results_df.index:
        ...
        adata_temp.obsm[coord_keys[2]] = np.vstack(
            (adata_temp.obs[coord_keys[0]].values, adata_temp.obs[coord_keys[1]].values)
        ).T
        ...
        if save_fig:
            ...
            print(
                f"Saving figure at {os.path.join(save_path, f'{dataset_name}_{params_name}_spatialmap.png')}"
            )
            fig.savefig(
                os.path.join(save_path, f"{dataset_name}_{params_name}_spatialmap.png")
            )
        ...
        plot_labels_seperately(
            ...
            save_path=os.path.join(
                save_path, f"{dataset_name}_{params_name}_clusters_seperate.png"
            ),
        )
        ...
        if plot_dot_plot or plot_heat_map:
            ...
            groups_subset = [
                ...
                if str(n) in adata_temp.obs[label_name].cat.categories
            ]
        ...
        if save_fullfig:
            ...
            print(
                f"Saving full-figure at {os.path.join(save_path, f'{dataset_name}_{params_name}_full_figure.png')}"
            )
            fig.savefig(
                os.path.join(save_path, f"{dataset_name}_{params_name}_full_figure.png")
            )


def _initialize_main_figure(
    main_figsize=(15, 9),
    ...
    grid = fig.add_gridspec(
        ncols=4, nrows=3, width_ratios=width_ratios, height_ratios=height_ratios
    )
...
def _plot_labels(
    ...
    scatterplot = ax_locs.scatter(
        adata_temp.obsm[key][:, 0],
